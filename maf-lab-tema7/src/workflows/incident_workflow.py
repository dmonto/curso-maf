...
def calculate_priority(
    ...
    if "caído" in text or "caido" in text or "error 500" in text:
        if users_affected is not None and users_affected >= 5:
            return Priority.P1, "Servicio crítico con caída o error severo para varios usuarios."
        return Priority.P2, "Servicio con error severo, pero impacto limitado o no confirmado."

    if "lento" in text or "degradado" in text:
        return Priority.P2, "Servicio degradado con impacto operativo."

    if service == ServiceName.UNKNOWN:
        return Priority.P3, "No se ha identificado un servicio concreto."
...
def calculate_sla_deadline(priority: Priority) -> str:
    ...
    deadline = now + timedelta(hours=hours_by_priority[priority])
...
async def draft_ticket(
    ...
    ctx: WorkflowContext[Never, TicketDraft],
) -> None:
    ...
    description = (
        f"Servicio afectado: {message.service.value}\n"
    ...
    if message.missing_fields:
        description += (
            "\nCampos pendientes antes de crear el ticket real: "
            + ", ".join(message.missing_fields)
        )
    ...
    ticket = TicketDraft(
        ...
        ready_for_creation=message.is_ready_for_ticket,
    )

    await ctx.yield_output(ticket)
...
def build_incident_workflow():
    workflow = (
        ...
        .add_edge(validate_required_data, draft_ticket)
        .build()
    )
...
async def main() -> None:
    ...
    request = IncidentInput(
        description="El ERP devuelve error 500 desde hace 10 minutos.",
